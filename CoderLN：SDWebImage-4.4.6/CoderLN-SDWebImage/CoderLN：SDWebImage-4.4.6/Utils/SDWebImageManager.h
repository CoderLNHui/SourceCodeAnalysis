...
typedef NS_OPTIONS(NSUInteger, SDWebImageOptions) {
     ...
     * By default, when you use `SDWebImageTransition` to do some view transition after the image load finished, this transition is only applied for image download from the network. This mask can force to apply view transition for memory and disk cache as well.
     默认情况下，当图像加载完成后使用`SDWebImageTransition`完成视图转换时，此转换仅适用于从网络下载图像。 这个掩码也可以强制为内存和磁盘缓存应用视图转换
     */
...
@property (strong, nonatomic, readonly, nullable) SDWebImageDownloader *imageDownloader;//图片下载工具类
 ...
 */
@property (nonatomic, copy, nullable) SDWebImageCacheKeyFilterBlock cacheKeyFilter;//缓存过滤器
