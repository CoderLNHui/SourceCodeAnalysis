...
#pragma mark - 逻辑层：SDWebImageManager_4.3.3 加载图片核心方法
/**
  调度图片的下载(Downloader)和缓存(Cache)，并不依托于 UIView+WebCache，完全可单独使用
    该方法内部，调用了 imageCache的根据URLKEY查找对应的图片缓存是否存在方法。还调用了 imageDownloader的使用下载器下载图片的方法。
    并不依托于 UIView+WebCache，完全可单独使用。
